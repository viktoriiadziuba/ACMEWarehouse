...
@Route(value = "searchemployee", registerAtStartup = false)
public class SearchEmployee extends VerticalLayout {
	...
	public SearchEmployee() {
		...
		search.addClickListener(e -> getEmployeeById(filterById.getValue().intValue()));
		
		grid.setColumns("id", "department", "person");
		grid.getColumnByKey("id").setFlexGrow(0);
		grid.getColumnByKey("department").setTextAlign(ColumnTextAlign.CENTER);
		grid.getColumnByKey("person").setTextAlign(ColumnTextAlign.CENTER);
