...
@Route(value = "searchstorage", registerAtStartup = false)
public class SearchStorage extends VerticalLayout {
	...
	public SearchStorage() {
		...
		Span title = new Span("Storage search");
		...
		search.addClickListener(e -> getStorageById(filterById.getValue().intValue()));
		
		grid.setColumns("id", "address");
		grid.getColumnByKey("id").setFlexGrow(0);
		grid.getColumnByKey("address").setTextAlign(ColumnTextAlign.CENTER);
		...
		// FOOTER
		Tab actionButton1 = new Tab(VaadinIcon.PACKAGE.create(), new RouterLink("Storages", StoragesView.class));
	...
	public void getStorageById(int id) {
