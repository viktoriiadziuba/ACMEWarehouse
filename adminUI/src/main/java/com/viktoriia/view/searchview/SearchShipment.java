...
@Route(value = "searchshipment", registerAtStartup = false)
public class SearchShipment extends VerticalLayout {

	private ShipmentService service = new ShipmentService();
	private Grid<Shipment> grid = new Grid<>(Shipment.class);
	
	public SearchShipment() {
		// HEADER
		Icon drawer = VaadinIcon.SEARCH.create();
		Span title = new Span("Shipment search");
		...
		search.addClickListener(e -> getShipmentById(filterById.getValue().intValue()));
		
		grid.setColumns("id", "quantity", "dateOfShipment", "description", "state");
		grid.getColumnByKey("id").setFlexGrow(0);
		grid.getColumnByKey("quantity").setTextAlign(ColumnTextAlign.CENTER);
		grid.getColumnByKey("dateOfShipment").setTextAlign(ColumnTextAlign.CENTER);
		...
		// FOOTER
		Tab actionButton1 = new Tab(VaadinIcon.TRUCK.create(), new RouterLink("Shipments", ShipmentsView.class));
