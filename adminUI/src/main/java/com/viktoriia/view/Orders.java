...
import com.viktoriia.view.addview.AddOrder;
...
public class Orders extends VerticalLayout {
	...
	public Orders() {
		...
		Tab addOrder = new Tab(VaadinIcon.CLIPBOARD_TEXT.create(), new RouterLink("Create Order", AddOrder.class));
		...
		HorizontalLayout header = new HorizontalLayout(drawer, title, addOrder, home, exit);
		header.expand(title);
	...
	private Component createCard() {
		...
		grid.setColumns("id", "description", "state");
